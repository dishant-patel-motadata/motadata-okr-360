...
export default function EmployeeList() {
  ...
  const { data, isLoading } = useEmployees({ page, limit: 20, search: search || undefined, department: department || undefined, group_name: groupName || undefined, is_active: true });
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        actions={
          <Button variant="outline" onClick={handleSync} disabled={syncEmployees.isPending}>
            <RefreshCw className={`mr-2 h-4 w-4 ${syncEmployees.isPending ? 'animate-spin' : ''}`} />
            Sync from AD
          </Button>
        }
      />
              ...
              <TableBody>
                {employees.map((emp) => (
                  <TableRow key={emp.employee_id}>
                    ...
                    <TableCell><Badge variant="outline">{emp.group_name}</Badge></TableCell>
                    <TableCell>
                      <Badge variant={emp.is_active ? 'default' : 'secondary'} className={emp.is_active ? 'bg-rating-outstanding-bg text-rating-outstanding' : ''}>
                        {emp.is_active ? 'Active' : 'Inactive'}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right">
                      <Button asChild variant="ghost" size="sm"><Link to={`/admin/employees/${emp.employee_id}`}>Edit</Link></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
